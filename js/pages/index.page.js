...
window.undoLastDrink = async (el) => {
  ...
  if (last.batch_id) {
    ...
    if (!bErr && b) {
      ...
      await supabase.from('stock_batches')
        .insert([{ product_id: last.product_id, quantity: 1, price_per_piece: last.price_at_purchase }]);
    }
  } else {
    await supabase.from('stock_batches')
      .insert([{ product_id: last.product_id, quantity: 1, price_per_piece: last.price_at_purchase ?? 0 }]);
  }
