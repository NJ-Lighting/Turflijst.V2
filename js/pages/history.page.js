...
async function loadUsers() {
  ...
  if ($('#h-user')) $('#h-user').innerHTML = options;
}
...
export async function loadHistory() {
  ...
  (data || [])
    ...
    .forEach(r => {
      ...
      const dtTxt = (dt.toLocaleString?.('nl-NL')) || dt.toISOString();
      const user = r?.users?.name || 'Onbekend';
      const prod = r?.products?.name || '—';
      const price = Number(r?.price_at_purchase ?? r?.products?.price ?? 0) || 0;
      const paidIcon = r?.paid ? '✔' : '✖';
      sum += price;
