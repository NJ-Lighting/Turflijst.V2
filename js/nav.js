...
(function () {
  ...
  if (!adminTrigger) {
    // Maak een aparte Admin-trigger als die nog niet bestaat
  ...
  if (!adminRow) {
    ...
    adminRow.innerHTML = `
      ...
      <div class="nav-admin-links"
           style="display:flex;flex-direction:column;gap:.3rem;padding-left:.5rem;"></div>
    `;
  }

  // Zorg dat adminRow direct na de Admin-knop staat
  if (adminTrigger.nextSibling) {
    drawer.insertBefore(adminRow, adminTrigger.nextSibling);
  } else {
  ...
  const ensureLink = (href, label) => {
    const exists = Array
      .from(adminLinksContainer.querySelectorAll('a'))
      .some((a) => (a.getAttribute('href') || '').toLowerCase() === href);
